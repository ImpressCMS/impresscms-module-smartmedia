...
*/		
		...
		$clipObj->setVar('presentation_file', $_POST['presentation_file']);
		$clipObj->setVar('presentation_formatid', $_POST['presentation_formatid']);
	...
	// We don't need this anymore, we'll also have to remove it from the db
	//$clipObj->setTextVar('block_title', $_POST['block_title']);
...
function editclip($showmenu = false, $clipid = 0, $folderid = 0)
{
	...
	// title
	$sform->addElement(new XoopsFormText(_AM_SMEDIA_CLIP_TITLE, 'title', 50, 255, $clipObj->title('e')), true);
	
	// We don't need this anymore, we'll also have to remove it from the db
	// block_title
	/*$block_title_text = new XoopsFormText(_AM_SMEDIA_CLIP_BLOCK_TITLE, 'block_title', 50, 255, $clipObj->block_title('e'));
	$block_title_text->setDescription(_AM_SMEDIA_CLIP_BLOCK_TITLE_DSC);
	$sform->addElement($block_title_text, false);
	*/
	// Description
	...
	// presentation_file
	$presentation_file_text = new XoopsFormText(_AM_SMEDIA_CLIP_PRESENTATION_FILE, 'presentation_file', 50, 255, $clipObj->presentation_file(), false);
	$presentation_file_text->setDescription(_AM_SMEDIA_CLIP_PRESENTATION_FILE);
	$sform->addElement($presentation_file_text);

	// presentation_formatid
	$presentation_formatid_select = new XoopsFormSelect( _AM_SMEDIA_CLIP_PRESENTATION_FORMATID, 'presentation_formatid', $clipObj->presentation_formatid() );
	$presentation_formatid_select->addOptionArray(smartmedia_getFormatArray(true));
	$sform -> addElement( $presentation_formatid_select );	
	...
	// tab_text_1
	$tab1_text = new XoopsFormTextArea(_AM_SMEDIA_CLIP_TAB_TEXT_1, 'tab_text_1', $clipObj->tab_text_1('e'), 20, 60);
	...
	// tab_text_2
	$tab2_text = new XoopsFormTextArea(_AM_SMEDIA_CLIP_TAB_TEXT_2, 'tab_text_2', $clipObj->tab_text_2('e'), 20, 60);
	...
	// tab_text_3
	$tab3_text = new XoopsFormTextArea(_AM_SMEDIA_CLIP_TAB_TEXT_3, 'tab_text_3', $clipObj->tab_text_3('e'), 20, 60);
	...
	// file_lr
	$lores = new XoopsFormText(_AM_SMEDIA_CLIP_FILE_LR, 'file_lr', 50, 10000, $clipObj->file_lr(), false);
	$lores->setDescription(_AM_SMEDIA_CLIP_FILE_LRDSC);
	$sform->addElement($lores);
	
	// file_hr
	$hires = new XoopsFormText(_AM_SMEDIA_CLIP_FILE_HR, 'file_hr', 50, 10000, $clipObj->file_hr(), false);
