...
class SmartmediaClip extends XoopsObject {
    ...
    */		
	function SmartmediaClip($languageid='default', $id = null) 
	{
        ...
        $this->initVar('presentation_file', XOBJ_DTYPE_TXTBOX, null, false);
        $this->initVar('presentation_formatid', XOBJ_DTYPE_INT, 0, false);
    ...
    */		
	function loadLanguage($languageid)
    {
    	...
    	if (!$this->clip_text) {
			...
			if ($default_clip_text) {
				...
				$this->clip_text->setVar('block_title', $default_clip_text->block_title());
	...
	/**
	 * @param string $ format to use for the output
	 * @return string presentation file url
	 */		
	function presentation_file($format="S")
	{
		return $this->getVar("presentation_file", $format);
	}			
	
	/**
	 * @param string $ format to use for the output
	 * @return string presentation format id
	 */		
	function presentation_formatid($format="S")
	{
		return $this->getVar("presentation_formatid", $format);
	}				
	...
	function title($format="S")
    {
     ...
     /**
	 * Returns the block title of the clip (used in the List of clips block)
	 *
	 * If the format is "clean", the title will be return, striped from any html tag. This clean
	 * title is likely to be used in the page title meta tag or any other place that requires 
	 * "html less" text
	 ...
	 * @return string title of the clip
	 */	
	function block_title($format="S")
    {
		$myts =& MyTextSanitizer::getInstance();
    	if ((strtolower($format) == 's') || (strtolower($format) == 'show')) {
    		return $myts->undoHtmlSpecialChars($this->clip_text->getVar("block_title", 'e' ), 1);
		} elseif ((strtolower($format) == 'clean')) {
			return smartmedia_metagen_html2text($myts->undoHtmlSpecialChars($this->clip_text->getVar("block_title")));
		} else {
			return $this->clip_text->getVar("block_title", $format);	
		}
    }	
	...
	/**
	 * Render the template of the presentation associated with this clip
	 *
	 * This method creates the format object associated to the presentation's format and then renders
	 * the template of the format
     *
	 * @return string template of the presentation's format
	 * @see SmartmediaFormat
	 * @see SmartmediaFormat::render()
	 */			
	function renderPresentationTemplate()
	{
		$smartmedia_format_handler = xoops_getmodulehandler('format', 'smartmedia');
		$formatObj = $smartmedia_format_handler->get($this->presentation_formatid());
		
		return $formatObj->render($this->presentation_file());		
		//return $formatObj->render('http://www.risq.qc.ca/risq2004/actes/27_14h00_Rene_Hatem.swf', '300', '300', true);		
		//return $formatObj->render('http://localhost/smart/bareau.swf', '300', '300', true);		
	...
	function renderTemplate()
	{
		...
		return $formatObj->render($this->file_lr(), $this->width(), $this->height(), $this->autostart());
	}
   ...
   *
	...
	function toArray($categoryid, $max_title_length=0, $forBlock=false) {
		...
		if ($this->presentation_file()) {
			$clip['presentation_file'] = $this->presentation_file();
			$clip['presentation'] = $this->renderPresentationTemplate();
		}
...
class SmartmediaClipHandler extends XoopsObjectHandler {
    ...
    */
    ...
    function &getClipsFromAdmin($start=0, $limit=0, $sort='clipid', $order='ASC', $languagesel)
    {
    	...
    	$ret    = array();
        $sql    =  "SELECT DISTINCT clips.clipid, clips.weight, clips_text.title, clips.counter, clips.created_date, clips.created_uid, folders.folderid, folders_text.title AS foldertitle, categories.categoryid
					FROM (
       ...
       $result = $this->_db->query($sql, $limit, $start);
        ...
        while ($myrow = $this->_db->fetchArray($result)) {
            ...
            $item['counter'] = $myrow['counter'];
            $item['folderid'] = $myrow['folderid'];
            $item['foldertitle'] = $myrow['foldertitle'];
            $item['created_date'] = $myrow['created_date'];
            $item['created_uid'] = $myrow['created_uid'];
            $item['categoryid'] = $myrow['categoryid'];
	...
	*/
	function insert(&$obj, $force = false)
	{
		...
		if ($obj->isNew()) {
			...
			$this->_db->quoteString($presentation_file),
			$presentation_formatid);
		} else {
