...
class SmartmediaFormat extends XoopsObject
{
    ...
    */	
	 ...
	 */	
	function render($url, $width="400", $height="324", $autostart="true")
	{
		$temp = $this->template();
		
		$temp = str_replace('{CLIP_URL}', $url, $temp); 
		if (!$width) {
			$temp = str_replace('width="{CLIP_WIDTH}"', "", $temp); 
		} else {
			$temp = str_replace('{CLIP_WIDTH}', $width, $temp); 
		}		
		if (!$width) {
			$temp = str_replace('height="{CLIP_HEIGHT}"', "", $temp); 
		} else {
			$temp = str_replace('{CLIP_HEIGHT}', $height, $temp); 
		}		
		
		$temp = str_replace('{CLIP_AUTOPLAY}', $autostart, $temp); 
