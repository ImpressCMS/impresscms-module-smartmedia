...
function upgradeDB()
{
    ...
    case '0.85':
        set_time_limit(60);
        printf("<h3>". _AM_SMEDIA_DB_UPDATE_TO."</h3>", '0.86' );
        echo "<ul>";

        // Add fields in smartmedia_clips
        $table = new SmartmediaTable('smartmedia_clips_text');
        $table->addAlteredField('block_title', "varchar(100) NOT NULL default ''");
        $ret = $dbupdater->updateTable($table) && $ret;
        unset($table);
    ...
    case '0.86':
        set_time_limit(60);
        printf("<h3>". _AM_SMEDIA_DB_UPDATE_TO."</h3>", '0.87' );
        echo "<ul>";

        echo "</ul>";

        set_time_limit(60);
        printf("<h3>". _AM_SMEDIA_DB_UPDATE_TO."</h3>", '0.88' );
        echo "<ul>";

        // Add fields in smartmedia_clips
        $table = new SmartmediaTable('smartmedia_clips');
        $table->addAlteredField('presentation_file', "text NOT NULL");
        $table->addAlteredField('presentation_formatid', "int(11) NOT NULL default '0'");
        $ret = $dbupdater->updateTable($table) && $ret;
        unset($table);
